...
# --- THE SETTINGS ---
MY_GMAIL = "olaniyitestimony996@gmail.com" 
APP_PASSWORD = "xmqskvuyzhiezpvj" 
...
def send_mail(to_email, subject, body):
    msg = MIMEMultipart()
    msg['From'] = f"{BRAND_NAME} <{MY_GMAIL}>"
    msg['To'] = to_email
    msg['Subject'] = subject
    msg.attach(MIMEText(body, 'plain'))
    
    # Securely managing the connection lifecycle to prevent drops
    server = None
    try:
        ...
        print(f"❌ Mail Error for {to_email}: {e}")
    finally:
        if server:
            try:
                server.quit()
...
def submit():
    ...
    # Diagnostic helper prints in your terminal window to track values
    print(f"📬 Received Form Submission - Name: {name}, Email from form: {user_email}")
    
    try:
        # 1. Sends the email to YOU
        send_mail(MY_GMAIL, f"New Order: {name}", f"Project received from {name}")

        # 2. Sends the confirmation email to the CLIENT
        if user_email:
            customer_msg = f"Hello {name},\n\nThank you for reaching out to {BRAND_NAME}! We have successfully received your project details.\n\nOur team will review your brief and get back to you shortly.\n\nBest regards,\n{BRAND_NAME} Team"
            send_mail(user_email, f"Project Received! - {BRAND_NAME}", customer_msg)
        else:
            print("⚠️ Warning: user_email variable was empty, skipping client email.")
